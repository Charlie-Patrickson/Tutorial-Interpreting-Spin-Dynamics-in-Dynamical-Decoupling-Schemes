...
def Plot_Bloch_trajectories(i, Signal_color, linewidth, ax, arrowhead_size, data, drive_colour_x, drive_colour_z, Signal_Max_Y, spin_colours='k', ax2 = None, ax3 = None):
    ...
    for df in data:
        ...
        if z[i]< 0:
            ax.plot(x.iloc[0:i+3], y.iloc[0:i+3], z.iloc[0:i+3], linewidth=linewidth, color = spin_colours[index])
            ax.set_title(f"t = {time[i]:.3f}")
            
        if z[i] >= 0:
            ax.plot(x.iloc[0:i-5], y.iloc[0:i-5], z.iloc[0:i-5], linewidth=linewidth, color = spin_colours[index])
            ax.set_title(f"t = {time[i]:.3f}")
        ...
        a = Arrow3D([0, x.iloc[i]], [0, y.iloc[i]], [0, z.iloc[i]], **arrow_prop_dict)
        ax.add_artist(a)
        ...
        if "Hx" in df.columns:
            ...
            if ax3:
                ax3.plot(time[0:i], drive_z[0:i], color = drive_colour_z)
