...
def Plot_Bloch_trajectories(i, Signal_color, linewidth, ax, arrowhead_size, data, drive_colour_x, drive_colour_z, Signal_Max_Y, spin_colours='k', ax2 = None, ax3 = None):
    ...
    for df in data:
        ...
        average_S_x.append(x[0:i+1])
        average_S_y.append(y[0:i+1])
        average_S_z.append(z[0:i+1])
        ...
        if ax2:
            ax2.plot(time[0:i], z[0:i], color = spin_colours[index])
        ...
        if "Hx" in df.columns:
            ...
            if ax3:
                ax3.plot(time[0:i], drive_x[0:i], color = drive_colour_x)
    ...
    average_S_x = (np.array(average_S_x)).mean(axis=0)
    average_S_y = (np.array(average_S_y)).mean(axis=0)
    average_S_z = (np.array(average_S_z)).mean(axis=0)
    
    ax.plot(average_S_x[0:i+1], average_S_y[0:i+1], average_S_z[0:i+1], linewidth=linewidth, color = spin_colours[index])
    ax.set_title(f"t = {time[i]:.3f}")
    
    arrow_prop_dict = dict(arrowstyle='-|>', linewidth = linewidth*2, mutation_scale=arrowhead_size, color = spin_colours[index], shrinkA=0, shrinkB=0)
    a = Arrow3D([0, average_S_x[i]], [0, average_S_y[i]], [0, average_S_z[i]], **arrow_prop_dict)
    ax.add_artist(a)
