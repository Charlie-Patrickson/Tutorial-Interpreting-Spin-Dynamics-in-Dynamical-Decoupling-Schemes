...
def Plot_Bloch_trajectories(i, Signal_color, linewidth, ax, arrowhead_size, data, drive_colour_x, drive_colour_z, Signal_Max_Y, spin_colours='k', ax2 = None, ax3 = None):
    ...
    for df in data:
        ...
        if ax2:
            ax2.plot(time[0:f], z[0:f], color = spin_colours[index])
            
        if ax3:
            ax3.plot(time[0:f], x[0:f], color = spin_colours[index])
        ...
        if "Hx" in df.columns:
            ...
            if abs(drive_y[i]) > 0.05:
            
                arrow_prop_dict = dict(arrowstyle='-|>', linewidth = linewidth*2, mutation_scale=arrowhead_size, color='k', shrinkA=0, shrinkB=0)
