...
data = pd.read_csv("Fig_5a_SimData_1.csv")
Noisy_data = pd.read_csv("Fig_5a_SimData_2.csv")
data = [data, Noisy_data]
Spin_colours = ['#7701A8', '#EE7C50', 'grey', 'darkgrey', 'lightgrey']
Detunings = [r"$0\pi$", r"$0.1\pi$"]

Total_No_of_data_points = 855
...
for i in range(0, Total_No_of_data_points, Skip_lines+1):
    ...
    ax2a.set_ylabel(r"$H_{Spin Lock}^{\prime}$", fontsize=18)
    ...
    ax3a.set_ylabel(r"$H_{Spin Lock}^{\prime}$", fontsize=18)
...
imageio.mimsave("Fig_5a.gif", images, duration=50)  # Adjust duration as needed
'''
with imageio.get_writer("Fig_5a.gif", mode="I", duration=60) as writer:
